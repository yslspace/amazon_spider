...
# 设置列表页URL的固定部分
url1 = 'https://www.amazon.fr/TOGUARD-Voiture-Embarquée-Objectif-Réglable/product-reviews/'
url2 = '/ref=cm_cr_arp_d_viewopt_rvwer?ie=UTF8&reviewerType=avp_only_reviews&formatType=current_format&pageNumber='
...
# 设置请求头部信息
headers1 = {
    'user-agent': 'Mozilla/5.0 (X11; CrOS x86_64 14.4.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.0.2981.0 Safari/537.36'}
...
for i in range(1, nrows):
    ...
    page = 1
    print('爬取：' + str(asin))
    print('爬取页面首页：' + str(url1 + asin + url2 + str(page)))
    results_this_asin = []
    headers = {'user-agent': choice(headerslist)}
    for j in range(1, 1000):
        time.sleep(2.0)
        a = (url1 + asin + url2 + str(page))
        ...
        if len(results_this_page) == 0 and page != 501:
            ...
            reviews.to_csv('reviews-fr-error.csv', index=False)
            sys.exit()
        else:
            pass
    ...
    reviews = pd.DataFrame(results)
    reviews.to_csv('reviews-fr.csv', index=False)
end_time = time.time()
