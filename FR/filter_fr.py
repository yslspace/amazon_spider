...
url0 = 'https://www.amazon.fr/dp/'
url1 = 'https://www.amazon.fr/Russell-Hobbs-Bouilloire-Electrique-Ebullition/product-reviews/'
url2 = '/ref=cm_cr_arp_d_viewopt_rvwer?ie=UTF8&reviewerType=avp_only_reviews&formatType=current_format&pageNumber=1'

file_path = 'asin-fr.xlsx'
...
def get_message(asin, list_switch):
    ...
    for c1 in fmt_vrp_reviews:
        try:
            ...
            fmt_vrp_review = contents1.split(' ', 3)[3]
    ...
    result.append({
        ...
        "fmt_vrp_review": fmt_vrp_review.strip('sur ').strip(' commentaires')
    })
    results = pd.DataFrame(result)
    results.to_excel('reviewscount-fr.xlsx', index=False)
