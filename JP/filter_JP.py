...
headers = {
    'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/534.23 (KHTML, like Gecko) Ubuntu/10.10 Chromium/11.0.686.1 Chrome/11.0.686.1 Safari/534.23'}
proxies = {'proxy': 'http://116.212.157.23:56354'}
url0 = 'https://www.amazon.co.jp/dp/'
url1 = 'https://www.amazon.co.jp/JBL-Studio-2-0chホームシアターシステムBluetooth-JBLBARSBLKJN-メーカー1年保証付き】/product-reviews/'
...
def get_message(asin, list_switch):
    ...
    item['img'] = strip(html0.xpath(".//img[@id='landingImage']/@src"))
    ...
    for c1 in fmt_vrp_reviews:
        try:
            ...
            fmt_vrp_review = contents1.split(' ', 3)[0]
            print(fmt_vrp_review)
    ...
    results = pd.DataFrame(result)
    results.to_excel('reviewscount-jp.xlsx', index=False)
