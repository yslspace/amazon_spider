...
for i in range(nrows):
    ...
    if i == 0:
        ...
        if r1.status_code in [304, 503]:
            ...
            reviews.to_excel('reviewscount-jp-error.xlsx', index=False)
            sys.exit()
        else:
            pass
        ...
        for c1 in fmt_vrp_reviews:
            try:
                ...
                fmt_vrp_review = contents1.split(' ', 1)[0]
        ...
        for c2 in fmt_all_reviews:
            try:
                ...
                fmt_all_review = contents2.split(' ', 1)[0]

            except:
                print('error')
                continue
        print('认证评论：' + str(fmt_vrp_review) + '   所有评论：' + str(fmt_all_review))
        results.append({
            "country": country,
            "asin": asin,
            "fmt_vrp_review": fmt_vrp_review,
            "fmt_all_review": fmt_all_review
        })

reviews = pd.DataFrame(results)
reviews.to_excel('reviewscount-jp.xlsx', index=False)
