...
# 设置列表页URL的固定部分
url1 = 'https://www.amazon.it/Telecamera-Rilevazione-Movimento-Registrazione-Parcheggio/product-reviews/'
...
# 设置请求头部信息
headers1 = {'user-agent': 'Mozilla/4.0 (compatible; MSIE 6.0; Windows 98; Win 9x 4.90)'}
...
for i in range(1, nrows):
    ...
    for j in range(1, 1000):
        ...
        if len(results_this_page) == 0 and page != 501:
            ...
            # reviews.to_csv('reviews-it-error.csv', index=False)
            sys.exit()
        else:
            pass
    ...
    reviews = pd.DataFrame(results)
    reviews.to_csv('reviews-it.csv', index=False)
end_time = time.time()
