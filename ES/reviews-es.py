...
# 设置列表页URL的固定部分
url1 = 'https://www.amazon.es/Grabadora-Estacionamiento-Detección-Movimiento-Grabación/product-reviews/'
...
for i in range(1, nrows):
    ...
    for j in range(1, 1000):
        ...
        if len(results_this_page) == 0 and page != 501:
            ...
            reviews.to_csv('reviews-es-error.csv', index=False)
            sys.exit()
        else:
            pass
    ...
    reviews = pd.DataFrame(results)
    reviews.to_csv('reviews-es.csv', index=False)
end_time = time.time()
