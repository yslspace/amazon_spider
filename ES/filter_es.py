...
headers = {
    'user-agent': 'Mozilla/5.0 (Macintosh; PPC Mac OS X) AppleWebKit/534.34 (KHTML, like Gecko) PhantomJS/1.9.7 Safari/534.34'}
proxies = {'proxy': 'http://116.212.157.23:56354'}
url0 = 'https://www.amazon.es/dp/'
url1 = 'https://www.amazon.es/Aigostar-Sea-30CDW-automático-protección/product-reviews/'
url2 = '/ref=cm_cr_arp_d_viewopt_rvwer?ie=UTF8&reviewerType=avp_only_reviews&formatType=current_format&pageNumber=1'
url3 = '/ref=cm_cr_arp_d_viewopt_rvwer?ie=UTF8&reviewerType=all_reviews&pageNumber=1&formatType=current_format'

file_path = 'asin-es.xlsx'
...
def get_message(asin, list_switch):
    ...
    if r0.status_code in [304, 503] or r1.status_code in [304, 503]:
        ...
        # results.to_excel('reviewscount-de-error.xlsx', index=False)
        sys.exit()
    else:
        pass
    ...
    item['img'] = list(json.loads(html0.xpath(".//img[@id='landingImage']/@data-a-dynamic-image")[0]))[0] \
        if list(json.loads(html0.xpath(".//img[@id='landingImage']/@data-a-dynamic-image")[0]))[0] else ''
    ...
    results = pd.DataFrame(result)
    results.to_excel('reviewscount-es.xlsx', index=False)
